#include "InsideGpuMesh.h"

void InsideGpuMesh::setModes(GLenum polygonMode, GLenum renderMode) {
	this->polygonMode = polygonMode;
	this->drawMode = renderMode;
}

void InsideGpuMesh::initVao(MeshGeometry* geometry) {
...
void InsideGpuMesh::setShader(Shader *shader) {
	this->shader = shader;
	...
	this->uniform_iTime = glGetUniformLocation(shader->getProgramId(), "iTime");
	this->uniform_iResolution = glGetUniformLocation(shader->getProgramId(), "iResolution");
}

void InsideGpuMesh::render(mat4 *modelMatrix, mat4 *projectionMatrix, GLfloat currentTime, vec2 viewportResolution) {
	...
	if (this->uniform_iTime != -1) glUniform1f(this->uniform_iTime, currentTime);

	if (this->uniform_iResolution != -1) glUniform2f(this->uniform_iResolution, viewportResolution.x, viewportResolution.y);
